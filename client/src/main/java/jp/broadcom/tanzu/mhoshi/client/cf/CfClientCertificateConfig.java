...
class CfClientCertificateConfig {
	...
	@Bean
	AdvancedTlsX509KeyManager grpcKeyManager(SslBundles sslBundles,
	 ...
	 * {@link AdvancedTlsX509KeyManager} is called on every new TLS handshake. BoringSSL
	 * (the default) caches key material at context-build time and would ignore the key
	 * manager's dynamic updates.
	 */
	@Bean
	 ...
	 *
	 * <p>
	 * Because Spring gRPC's auto-configured {@code NamedChannelCredentialsProvider} is
	 ...
	 * full TLS handshake — without this, TLS 1.3 session-ticket resumption would cause
	 * the server to see the old client cert even on a fresh connection.</li>
	...
	ApplicationRunner certRotationListener(SslBundles sslBundles,
			@Value("${spring.grpc.client.channels.local.ssl.bundle}") String bundleName,
			@Value("${spring.ssl.bundle.pem.${spring.grpc.client.channels.local.ssl.bundle}.key.alias}") String aliasName,
			AdvancedTlsX509KeyManager keyManager, SslContext grpcClientSslContext,
	...
	private void updateKeyManager(AdvancedTlsX509KeyManager keyManager, SslBundle bundle, String aliasName)
